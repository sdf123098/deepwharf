<p align="center">
  <img src="resources/brand-logo.png" width="120" alt="DeepWharf 标志">
</p>
...
<p align="center">
  <img src="resources/pet/pet.png" width="250" alt="鲸鱼娘桌宠">
</p>

<p align="center">
  <em>DeepWharf 自带一只住在桌面上的鲸鱼娘——还会帮你盯着用量。</em>
</p>
...
应用图标是深蓝圆盘上的一只小白鲸，而桌宠是她的拟人化形象——戴贝雷帽的女仆鲸鱼娘，
身边围着一群小鲸鱼伙伴。

- **透明置顶**——默认停在主窗口右下角，始终浮在最上层。
- **有生命感**——待机漂浮、悬停凑近、单击 Q 弹并冒台词气泡。
- **可互动**——双击打开主窗口；右键菜单；拖拽移动（位置自动记住）。
- **举牌显示用量**——可选的牌子，实时显示 输入 / 输出 / 缓存命中率 / 上下文占用，
  数据来自官方 token-meter 投影。
- **随任务反应**——任务完成跳起来庆祝，Agent 出错摇头。
- **换装自由**——`npm run pet <png>` 从任意素材重新生成抠图（泛洪 + 光晕侵蚀 + 最大连通域）。
...
- **全局字体**——预设下拉（默认 HarmonyOS Sans SC），也可保留自定义字体；
  外壳与内嵌 Web UI 同步生效。
